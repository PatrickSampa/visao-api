    ...
    async execute(data: IGetInformationsFromSapiensDTO): Promise<any> {
        ...
        do {
            ...
            for (const tarefa of tarefas) {
                ...
                const pasta_id = tarefa.pasta.id;
                // console.log(tarefa)
                // console.log({pastaId: pasta_id, pessoaId })
                try {
                    
                    response.push(await solicitarDossiePrevidenciarioUseCase.execute({cookie, pastaId: pasta_id, pessoaId }));
