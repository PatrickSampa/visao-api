...
export async function  getInformaçoesIniciasDosBeneficiosSuperDosprev(paginaHTML_DOSPREV_Formatada: any): Promise<IBeneficiosDTO[]> {
    const result: IBeneficiosDTO[] = [];
    const valorMaximoparaPecoorerALinha = 20;
    const valorDaLinhaInicial = 1;
    for (let indexDaLinha = valorDaLinhaInicial; indexDaLinha <= valorMaximoparaPecoorerALinha; indexDaLinha++) {
        const xpathDaLinhaParaOElementoTipo = "/html/body/div/div[6]/table/tbody/tr[" + indexDaLinha + "]/td[6]";
        ...
        const xpathDaLinhaParaOElementoNB = "/html/body/div/div[6]/table/tbody/tr[" + indexDaLinha + "]/td[1]"
        const nb = (getXPathText(paginaHTML_DOSPREV_Formatada, xpathDaLinhaParaOElementoNB)).replace(/\D/g, '');
        
        const xpathDaLinhaParaOElementoBeneficio = "/html/body/div/div[6]/table//tr[" + indexDaLinha + "]/td[2]"
        const beneficio = getXPathText(paginaHTML_DOSPREV_Formatada, xpathDaLinhaParaOElementoBeneficio)
   
        const xpathDaLinhaParaOElementoDIB = "/html/body/div/div[6]/table/tbody/tr[" + indexDaLinha + "]/td[4]"
        const dib = correçaoDoErroDeFormatoDoSapiens(getXPathText(paginaHTML_DOSPREV_Formatada, xpathDaLinhaParaOElementoDIB));

        const xpathDaLinhaParaOElementoDCB = "/html/body/div/div[6]/table/tbody/tr[" + indexDaLinha + "]/td[5]"
